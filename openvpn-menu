...
install_openvpn() {
  ...
  if ! command -v openvpn &>/dev/null; then
    ...
    # Download and set up the firewall.sh script and related files
    echo "Setting up firewall.sh and configuration files..."
    mkdir -p /opt/openvpn
    wget -q -O "$FIREWALL_SCRIPT" https://www.qsos.uk/downloads/openvpn/firewall.sh
    touch "$PORTS_FILE" "$LOCAL_PORTS_FILE"
